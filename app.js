...
function productCard(p, rank, opts = {}) {
  ...
  return `
    <article class="product-card chip-${esc(p.category)}" data-pid="${esc(p.id)}">
      ...
      ${((p.where || []).length || p.region_note) ? `<div class="pc-bottom">${p.region_note ? `<span class="pc-region">🏷️ ${esc(p.region_note)}</span>` : ""}${(p.where || []).map((w) => `<span class="pc-where">${esc(w)}</span>`).join("")}</div>` : ""}
